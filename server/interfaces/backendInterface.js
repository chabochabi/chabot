...
class BackendInterface {
    ...
    constructor(dataManager, backtestManager, analysisManager, emitter, broadcaster) {
        this.dm = dataManager;
    ...
    async clientRequest(req, options) {
        ...
        switch (req) {
            case 'backtestSymbolsList':
                let data = { backtestSymbols: await this.bm.getBacktestSymbolList() }
                this.broadcast(req + ':' + options.symbol, data);
                break;

            case 'history':
                if (this.dm.hasSymbol(options.symbol, options.source)) {
                    let data = { klines: this.dm.readAll(options.symbol, options.source) };
                    this.broadcast(req + ':' + options.symbol, data);
                }
                break;
            ...
            case 'backtestHistory':
                if (this.dm.hasSymbol(options.symbol, 'backtest')) {
                    let data = { klines: this.dm.readAll(options.symbol, 'backtest') };
